...
func fire(mob_spawner, mob_spawn_pos, _player, tween) -> void:
	tween.tween_callback(func():
		...
		var left_positions: Positions = mob_spawn_pos.get_horizontal_positions("left", viewport_center_pos + 30)
		...
		var rigth_positions = mob_spawn_pos.get_horizontal_positions("rigth", viewport_center_pos - 30)
	...
	tween.tween_callback(func():
		...
		# parede de mobs lado esquerdo
		var left_positions: Positions = mob_spawn_pos.get_horizontal_positions("rigth", viewport_center_pos + 150)
		while left_positions.mob_position.y > 0:
			mob_spawner.spawn_mob_common(left_positions, 180, Vector2(130, 0), "walk")
			left_positions.set_y(left_positions.mob_position.y - 120)
		
		# parede de mobs lado direito
		var rigth_positions = mob_spawn_pos.get_horizontal_positions("left", viewport_center_pos - 150)
		while rigth_positions.mob_position.y < MyUtility.get_viewport().size.y:
